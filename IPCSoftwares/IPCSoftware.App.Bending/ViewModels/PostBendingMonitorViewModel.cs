    ...
    public class PostBendingMonitorViewModel : BaseViewModel, IDisposable
    {
        ...
        public void Initialize()
        {
            ...
            _liveDataPoller = new SafePollerEx(
                ...
                _logger,
                ex => _logger.LogError(
                    $"[PostBendingMonitor] Poller error: {ex.Message}",
                    LogType.Diagnostics),
                requestId: 29);
        ...
        private async Task UpdatePostBendingMonitorFromService(
            Dictionary<int, object> data)
        {
            try
            {
                ...
                {
                    var model =
                        Deserialize<PostBendingMonitorModel>(modelObj);
            ...
            {
                _logger.LogError(
                    $"[PostBendingMonitor] UpdatePostBendingMonitorFromService error: {ex.Message}",
                    LogType.Diagnostics);
            }
